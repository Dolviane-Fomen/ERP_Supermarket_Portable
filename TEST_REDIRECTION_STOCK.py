#!/usr/bin/env python3
"""
Script pour tester la redirection après création de facture d'achat
"""
...
def test_redirection_stock():
    """
    Test pour vérifier que la redirection fonctionne
    """
    print("🔍 TEST REDIRECTION APRÈS FACTURE D'ACHAT")
    print("=" * 60)
    ...
    try:
        ...
        fournisseur, created = Fournisseur.objects.get_or_create(
            intitule="Test Redirection",
            defaults={'agence': agence}
        )
        
        facture = FactureAchat.objects.create(
            numero_fournisseur="TEST_REDIRECTION",
            date_achat=timezone.now().date(),
            heure=timezone.now().time(),
            reference_achat=f"REDIRECTION_{int(timezone.now().timestamp())}",
            prix_total_global=1500.00,
            statut='validee',
        ...
        ligne = LigneFactureAchat.objects.create(
            ...
            prix_unitaire=214.29,
            quantite=quantite_ajoutee,
            prix_total_article=1500.00
        )
        ...
        MouvementStock.objects.create(
            ...
            facture_achat=facture,
            fournisseur=fournisseur,
            commentaire=f"Test Redirection - {facture.reference_achat}"
        )
        ...
        # 8. Simuler la redirection vers consulter_articles
        print(f"\n📋 SIMULATION REDIRECTION:")
        print(f"   - URL de redirection: /stock/consulter_articles/")
        ...
        if article.stock_actuel == stock_avant + quantite_ajoutee:
            ...
            print("✅ SUCCÈS: La redirection vers consulter_articles est configurée!")
            print("   Maintenant, après création d'une facture d'achat:")
            print("   1. Vous serez redirigé vers la liste des articles")
            print("   2. Vous verrez le message de succès")
            print("   3. Vous verrez le stock mis à jour automatiquement")
        else:
